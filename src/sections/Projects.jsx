    ...
    return (
        <section id="projects" className="py-24 relative overflow-hidden">
            <div className="container mx-auto px-6 realtive z-10">
                ...
                <div className="absolute bottom-1/4 left-0 2-64 h-64 bg-hightlight/5 rounded-full blur-3xl"/>
                             ...
                             style={{ animationDelay: `${(idx + 1) * 100}ms` }}>
                                ...
                                <div className="relative overflow-hidden aspect-video">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                        <a href={project.link} className="p-3 rounded-full glass hover:bg-primary/20 hover:text-secondary-foreground transition-all">
                                            <ArrowUpRight className="w-5 h-5"/>
                                        </a>
                                        <a href={project.github} className="p-3 rounded-full glass hover:bg-primary/20 hover:text-secondary-foreground transition-all">
                                ...
                                <div className="p-6 space-y-4"> 
                                    <div className="flex items-start justify-between">
                                        <h3 className="text-xl font-semibold group-hover:text-secondary-foreground transition-colors">{project.title}</h3>
                                        <ArrowUpRight className="w-5 h-5 
                                        text-muted-foreground group-hover:text-secondary-foreground
                                        group-hover:translate-x-1
                                        group-hober:-translate-y-1 transition-all"/>
                                    </div>
                                    <p className="text-muted-foreground text-sm">{project.description}</p>
                                    <div className="flex flex-wrap gap-2">{project.tags.map((tag, tagIdx) => (
                                        <span key={tagIdx} className="px-4 py-1.5 rounded-full bg-surface text-xs font-medium border border-border/50 text-muted-foreground hover:border-primary/50 hover:text-secondary-foreground transition-all duration-300 ">{tag}</span>
                                    ))}
                                    </div>
                                </div>
                            </div>
